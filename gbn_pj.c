...
// you can reset this for your own use
// but 64 is suggested
// this is not the same as the sliding window size
#define BUFSIZE 64
...
struct Sender
{
    int base;
    int nextseq;
    int window_size;            // this should be seen as the sliding window size
    float estimated_rtt;
    int buffer_next;
    ...
    while (1)
    {
        ...
        if (TRACE >= 2)
        {
            ...
            if (eventptr->evtype == 0)
                ...
                printf(", fromlayer3 ");
            char *entity_name = eventptr->eventity == 0 ? "A" : "B";
            printf(" entity: %s\n", entity_name);
        }
